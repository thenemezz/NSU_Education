...
public class ClientMessage {
    ...
    public enum ClientMessageStatus {
        ...
        USER_MESSAGE ("message", "message,session"),
        UNKNOWN ("", "");
    ...
    public static ClientMessage parseMessage(String message) {
        ...
        try {
            ...
            if (root.getElementsByTagName("message") != null && root.getElementsByTagName("message").getLength() > 0) {
                clientMessage.setMessage(root.getElementsByTagName("message").item(0).getTextContent());
            } 
            if (root.getElementsByTagName("name") != null && root.getElementsByTagName("name").getLength() > 0) {
                clientMessage.setName(root.getElementsByTagName("name").item(0).getTextContent());
            }
            if (root.getElementsByTagName("session") != null && root.getElementsByTagName("session").getLength() > 0) {
